...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    ...
    //更新当前用户头像
    const sql = `update users set imgUrl="${newname}" where id=${req.query.id}`;
    ...
    if (data.length) {
      ...
      const token =
        jwt.sign(userInfo, secretKey, {
          expiresIn: 60 * 60 * 24  //token的过期时间,单位是秒
        });
 ...
 * 修改账号头像
 */
router.post("/avataredit", (req, res) => {
...
router.post("/editpwd", (req, res) => {
  ...
  const sql = `update users set password="${newPwd}" where id=${id} and password='${oldPwd}' `;
  conn.query(sql, (err, data) => {
    if (err) throw err;
    if (data.affectedRows > 0) {
